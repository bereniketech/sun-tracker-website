...
function getCountdownState(now: Date, windows: Array<{ label: string; window: TimeWindow }>): CountdownState {
  ...
  return {
    status: "ended",
    label: "No more golden hour windows today",
    ms: 0,
  };
}

export function GoldenHourCountdown({ sunData }: GoldenHourCountdownProps) {
  ...
  const countdown = useMemo(() => {
    return getCountdownState(now, [
      { label: "Morning", window: sunData.goldenHour },
      { label: "Evening", window: sunData.goldenHourEvening },
    ]);
  }, [now, sunData.goldenHour, sunData.goldenHourEvening]);

  return (
    <article className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5">
      <div className="flex items-center justify-between gap-2">
        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-amber-900">Golden Hour</p>
        {countdown.status === "active" ? (
          <span className="rounded-full bg-amber-200 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.14em] text-amber-900">
            NOW
          </span>
        ) : null}
      </div>

      <p className="mt-1.5 text-sm font-semibold text-amber-950">{countdown.label}</p>
